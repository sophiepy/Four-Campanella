class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.save
        ...
        @items.each do |cart_item|
          ...
          @items.item_id = item.item.id
          @items.item.name = item.item.name
          @items.tex_excluded = item.item.price
  ...
  def show
    ...
    @order_item = @order.items #特定したorserからorder_items全部取得
    @total = 0 #変数提議　合計を計算する変数
  ...
  private
  def order_params
    params.require(:order).permit(:payment, :name, :address, :postage, :zip_code, :item_id, :price)
  end
